...
/**
 * Wraps another CachesProvider to make it sorted. Geocaches closer to the
 * provided center come first in the getCaches list. Until setCenter has been
 * called, the list will not be sorted.
 */
public class CachesProviderSorted implements ICachesProviderCenter, IDistanceAndBearingProvider {
    ...
    private WeakHashMap<Geocache, DistanceAndBearing> mDistances = new WeakHashMap<Geocache, DistanceAndBearing>();
    ...
    public DistanceAndBearing getDistanceAndBearing(Geocache cache) {
        ...
        if (d == null) {
            ...
            float bearing = (float)GeoUtils.bearing(mLatitude, mLongitude, cache.getLatitude(),
                    cache.getLongitude());
    ...
    /**
     * Updates mSortedList to a sorted version of the current underlying cache
     * list
     */
    ...
    public void setCenter(double latitude, double longitude) {
        if (isInitialized && Util.approxEquals(latitude, mLatitude)
    ...
    public double getFurthestCacheDistance() {
        if (!isInitialized) {
            Log.e("TreasureHunter", "getFurthestCacheDistance called before setCenter");
            return 0;
        }
        if (mSortedList == null || mCachesProvider.hasChanged()) {
            updateSortedList();
