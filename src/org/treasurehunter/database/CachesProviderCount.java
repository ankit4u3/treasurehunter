...
public class CachesProviderCount implements ICachesProviderCenter {
    ...
    public CachesProviderCount(ICachesProviderArea area, int minCount, int maxCount) {
    ...
    public GeocacheList getCaches() {
        if (mCachesProviderRadius.hasChanged()) {
            mCaches = null;
            mIsCountValid = false;
            // Need to do this check to avoid having the algorithm run amok
            int total = mCachesProviderArea.getTotalCount();
            if (total <= mMaxCount) {
                // Log.d("TreasureHunter",
                // "CachesProviderCount.getCaches: Total number is few enough");
        ...
        if (!mIsCountValid) {
            ...
            findRadius(mRadius);
            // Log.d("TreasureHunter", "CachesProviderCount calculated in " +
    ...
    private void findRadius(double radiusToTry) {
        ...
        // Log.d("TreasureHunter", "CachesProviderCount first count = " +
        // count);
        if (count > mMaxCount) {
            while (count > mMaxCount && iterationsLeft > 1) {
                ...
                iterationsLeft -= 1;
                // Log.d("TreasureHunter",
                // "CachesProviderCount search inward count = " + count);
            }
            ...
            while (count < mMinCount && radiusToTry < MAX_RADIUS / DISTANCE_MULTIPLIER
                ...
                iterationsLeft -= 1;
                // Log.d("TreasureHunter",
                // "CachesProviderCount search outward count = " + count);
            }
        }
        if (count < mMinCount && iterationsLeft > 0) {
            findWithinLimits(radiusToTry, radiusToTry * DISTANCE_MULTIPLIER, iterationsLeft);
        } else if (count > mMaxCount && iterationsLeft > 0) {
            findWithinLimits(radiusToTry / DISTANCE_MULTIPLIER, radiusToTry, iterationsLeft);
        } else {
            mCount = count;
            mRadius = radiusToTry;
        }
    ...
    private void findWithinLimits(double minRadius, double maxRadius, int iterationsLeft) {
        double radiusToTry = (minRadius + maxRadius) / 2.0;
        int count = countHitsUsingRadius(radiusToTry);
        if (count <= mMaxCount && count >= mMinCount) {
            // Log.d("TreasureHunter",
            // "CachesProviderCount.findWithinLimits: Found count = " + count);
            mCount = count;
            mRadius = radiusToTry;
            return;
        }
        if (iterationsLeft <= 1) {
            // Log.d("TreasureHunter",
            // "CachesProviderCount.findWithinLimits: Giving up with count = " +
            // count);
            mCount = count;
            mRadius = radiusToTry;
            return;
