...
public class AnimatorThread {
    ...
    void updateTime() {
        ...
        if (mCurrentTickDelta > 0.3) {
            Log.w("TreasureHunter", "Elapsed time " + mCurrentTickDelta + " capped at 0.3 sec");
        ...
        public void run() {
            ...
            while (mShouldRun) {
                ...
                try {
                    ...
                    if (c == null) {
                        Log.w(this.getClass().getName(), "run(): lockCanvas returned null");
                        continue;
                    }
        ...
        public void run() {
            ...
            while (mShouldRun && mRemainingRuns > 0) {
                ...
                try {
                    c = mSurfaceHolder.lockCanvas(null);
                    if (c == null) {
                        Log.w("TreasureHunter", "run(): lockCanvas returned null");
                        continue;
                    }
                    synchronized (mSurfaceHolder) {
            ...
            if (ticksDone == 1) {
                Log.d("TreasureHunter", "Iterated tick #" + mNextTickNo + " (@" + mSecsPerTick
                        + "s)");
            } else if (ticksDone > 0) {
                Log.d("TreasureHunter", "Iterated " + ticksDone + " ticks (@" + mSecsPerTick
                        + "s) in " + duration + "ms => " + (ticksDone * 1000 / duration) + "fps");
            }
    ...
    public AnimatorThread(SurfaceHolder surfaceHolder, ProximityPainter painter) {
    ...
    public void start() {
        ...
        Log.d("TreasureHunter", "AnimatorThread.start()");
    ...
    public void setupTiming(int nrOfTicks, double secondsPerTick) {
        ...
        Log.d("TreasureHunter", "setupTiming(" + secondsPerTick + " sec/tick, ticks=" + nrOfTicks
                + ")");
        mResumeAtTime = 0;
        // mShouldRun = true;
    ...
    /**
     * @param listener is invoked after the thread has stopped or at once if the
