...
public class ProximityPainter {
    ...
    private static int[] mMetricDistanceMarks = {
            10, 20, 50, 100, 500, 1000, 5000, 10000, 50000, 100000, 1000000
    };

    private static String[] mMetricDistanceTexts = {
            "10m", "20m", "50m", "100m", "500m", "1km", "5km", "10km", "50km", "100km", "1000km"
    };

    // Note, these numbers are not exact, but fall with 0-18 inches off starting
    // at 50 feet, this is neglectible - Mark Schneider.
    private static int[] mEnglishDistanceMarksInMeters = {
            15, 30, 76, 152, 304, 805, 1609, 8046, 16093, 80467, 160934, 1609344
    };

    private static String[] mEnglishDistanceTexts = {
            "50f", "100f", "250f", "500f", "1000f", "0.5M", "1M", "5M", "10M", "50M", "100M",
            "1000M"
    };

    private Parameter[] allParameters = {
            mDeviceDirection, mGpsAccuracy, mScaleFactor, mUserSpeed, mUserDirection, mLatitude,
            mLongitude
    };
    ...
    public void draw(Canvas canvas) {
        ...
        if (mUserSpeed.get() > 0) {
            ...
            int x7 = xRelativeUser(speedRadius,
                    Math.toRadians(mUserDirection.get() - direction - 90));
            int y7 = yRelativeUser(speedRadius,
                    Math.toRadians(mUserDirection.get() - direction - 90));
            canvas.drawLine(mCenterX, mUserY, x7, y7, mSpeedPaint);
        }

        int[] distanceMarks;
        String[] distanceTexts;

        if (mUseMetric) {
            distanceMarks = mMetricDistanceMarks;
            distanceTexts = mMetricDistanceTexts;
        } else {
            distanceMarks = mEnglishDistanceMarksInMeters;
            distanceTexts = mEnglishDistanceTexts;
        }
        ...
        if (mCachesProvider.hasChanged()) {
            Log.d("TreasureHunter", "ProximityPainter drawing " + caches.size() + " caches");
        ...
        for (Geocache geocache : caches) {
            ...
            if (geocache == mSelectedGeocache) {
                int glowTh = (int)(mCachePaint.getStrokeWidth() * 2); // (1.0 +
                                                                      // Math.abs(Math.sin(mTime))));
                drawGlow(canvas, x, y, (int)(cacheScreenRadius + mCachePaint.getStrokeWidth() / 2),
                        glowTh);
            }
    ...
    private void drawGlow(Canvas c, int x, int y, int radius, int thickness) {
        ...
        float[] positions = {
                0, ((float)radius) / (radius + thickness), 1f
        };
        // float[] positions = { 0, (radius +
        // 0.5f*thickness)/(radius+thickness), 1f };
        // float[] positions = { 0.5f, 0.9f, 1f };
        float drawRadius = radius + thickness * 1.5f;
        if (drawRadius <= 0)
            return; // Resolves crash (issue #77)
        // TODO: Wastes objects
        Shader glowShader;
        glowShader = new RadialGradient(x, y, drawRadius, colors, positions, TileMode.MIRROR);
