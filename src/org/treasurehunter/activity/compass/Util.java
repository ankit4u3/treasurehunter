...
public class Util {
    ...
    public static double[] decimalDegreesToDegreeMinutes(double fDegrees) {
        ...
        return new double[] {
                dAbsDegrees, fAbsMinutes
        };
    }

    public static String formatDegreesCoordsAsStringNWSE(double degLat, double degLon) {

        double[] minutesLat = decimalDegreesToDegreeMinutes(degLat);
        double[] minutesLon = decimalDegreesToDegreeMinutes(degLon);

        return String.format(Locale.US, (degLat < 0 ? "S" : "N") + " %1$d %2$06.3f "
                + (degLon < 0 ? "W" : "E") + " %3$d %4$06.3f", (int)minutesLat[0], minutesLat[1],
                (int)minutesLon[0], minutesLon[1]);
    }
    ...
    public static CharSequence formatDegreesAsDecimalDegreesString(double fDegrees) {
        ...
        return String.format(Locale.US, (fDegrees < 0 ? "-" : "") + "%1$d %2$06.3f",
                (int)degreesMinutes[0], degreesMinutes[1]);
    }
