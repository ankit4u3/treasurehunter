...
/**
 * Change LocationControlDi.create() to return an instance of this class to use
 * fake locations everywhere but in the Map view.
 */
public class GeoFixProviderFake implements GeoFixProvider {
    ...
    public static class FakeDataset {
        ...
        /**
         * The fake positions will be interpreted to be given relative to this
         * location
         */
        ...
        /**
         * A number of GeoFixes with values relative some starting point. The
         * first fix must have relative time 0
         */
        private GeoFix[] mFixes;

        public FakeDataset(int reportPeriodMillisec, double latStart, double lonStart,
        ...
        public GeoFix getGeoFixAtTime(long time) {
            ...
            if (relTime > mFixes[mFixes.length - 1].getTime()) {
                ...
                nextFix = mFixes[0];
                ratio = ((relTime - lastFix.getTime()) / (mLoopTime - lastFix.getTime()));
            } else {
                for (int i = 1; i < mFixes.length; i++) {
                    if (mFixes[i].getTime() == relTime) {
                        ...
                        ratio = (((float)relTime - lastFix.getTime()) / (nextFix.getTime() - lastFix
                                .getTime()));
                        break;
                    }
                }
            ...
            return new GeoFix(lastFix.getAccuracy(), lastFix.getAltitude()
                    + (nextFix.getAltitude() - lastFix.getAltitude()) * ratio, mLatStart
                    + lastFix.getLatitude() + (nextFix.getLatitude() - lastFix.getLatitude())
                    * ratio, mLonStart + lastFix.getLongitude()
                    + (nextFix.getLongitude() - lastFix.getLongitude()) * ratio, time,
                    (String)lastFix.getProvider());
        }
    }

    /** Move slowly between three locations */
    public static FakeDataset YOKOHAMA = new FakeDataset(2000, 35.4583162355423,
            139.62289574623108, 10000, new GeoFix[] {
    ...
    /** A single location */
    public static FakeDataset TOKYO = new FakeDataset(10000, 35.690448, 139.756225, 10000,
            new GeoFix[] {
                new GeoFix(16, 10, 0, 0, 0, "gps"),
            });

    /** Another single location */
    public static FakeDataset LINKOPING = new FakeDataset(10000, 58.398050, 15.612208, 10000,
            new GeoFix[] {
                new GeoFix(8, 20, 0, 0, 0, "gps"),
            });

    /** A stationary location slightly west of Link�ping */
    public static FakeDataset SLAKA = new FakeDataset(10000, 58.366667, 15.538333, 10000,
            new GeoFix[] {
                new GeoFix(8, 20, 0, 0, 0, "gps"),
            });

    /** A stationary location more west of Link�ping */
    public static FakeDataset MANTORP = new FakeDataset(10000, 58.35, 15.283333, 10000,
            new GeoFix[] {
                new GeoFix(8, 20, 0, 0, 0, "gps"),
            });

    /** Simulate moving at highway speeds and report location every second */
    public static FakeDataset CAR_JOURNEY = new FakeDataset(1000, 58.398050, 15.612208,
            70 * 60 * 1000, new GeoFix[] {
                    new GeoFix(8, 20, 0, 0, 0, "gps"),
                    new GeoFix(16, 30, 58.432729 - 58.398050, 15.669937 - 15.612208, 5 * 60 * 1000,
                            "gps"),
                    new GeoFix(32, 40, 58.591698 - 58.398050, 16.12896 - 15.612208, 35 * 60 * 1000,
                            "gps"),
                    new GeoFix(24, 30, 58.432729 - 58.398050, 15.669937 - 15.612208,
                            65 * 60 * 1000, "gps"),
            });
